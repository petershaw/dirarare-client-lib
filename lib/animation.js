...
Animation = module.exports = function animation(matrix) {
	...
	animation.animator = function animator(){
		_.each(animation.tasks, function(task){
			...
			task.temporarytimeconst = task.temporarytimeconst || task.time;
			if(typeof task.temporarytimeconst === 'function'){
				task.temporarytimeconst = task.temporarytimeconst();
			}

			if(task.elapsed >= parseInt(task.temporarytimeconst)){
				_.each(task.fnlist, function(fn){
					fn(task.element.element, animation.matrix);
				});
				task.elapsed -= parseInt(task.temporarytimeconst);
				task.temporarytimeconst = null;
			}
		});
		animation.fn()
	}
